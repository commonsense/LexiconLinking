...
replaced = {}
def replace_with_ids(seq,strings=False):
    if strings: #enforce string output
        return map(lambda x: (replaced.has_key(fnvhash([x])) and str(replaced[fnvhash([x])])) or str(x), seq)
    else:
...
def run_pspan(top_k, maxgap, minlen, totalruns, indicies,seen_seq = []):
    # execute pspan
    fname = "%s/%s"
    os.system("rm %s.new_keys; touch %s.new_keys; rm %s.map; touch %s.map; rm %s.replace; touch %s.replace" % (fname,fname,fname,fname,fname,fname))
    ...
    for seq_i in xrange(0,len(sequences)):
        for key, gaps in sorted(all_sequence_matches[seq_i].items(), key = lambda x: len(x[1]), reverse=True):
            seq_description = "Sequence %i: %s " % (seq_i,' / '.join(map(lambda x: keynames[x[1]]+(key == x[0] and "/_" or ""), enumerate(sequences[seq_i]))))
...
def test1():
    ...
    for i,fname in enumerate(file_index):
        ofile = open(fname, 'r')
        out_seq = ' '.join(replace_with_ids(map(lambda x: int(x.strip()), ofile.readlines()),strings=True))

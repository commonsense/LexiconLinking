...
static bool
parseSequenceMetaFile (const char* filename, std::vector<const SeqMine::Sequence*>& S,  Lexicon & L)
{
	...
	for (unsigned int seqId = 0 ; in.eof () == false ; ++seqId) {
		...
		SequenceT* seq = SequenceT::readFromFile (seqFilename, L);
		std::cout << "done 1" << std::endl;
...
main (int argc, char* argv[])
{
	...
	config.add_options ()
		("project,P", po::value<std::string>(&projectName),
			"Input name of project file")
	...
	po::notify (vm);
	if (vm.count ("help") || vm.count ("project") == 0 )//||	vm["input"].as<std::vector<std::string> >().size () != 1)
	{
		...
		std::cerr << std::endl;
		std::cerr << "Usage: pspan [options] -P project_directory" << std::endl;
		std::cerr << std::endl;
    ...
    std::string projectFilename = projectName+"/"+projectName+".proj";
	std::string outputFilename = projectName+"/"+projectName+".out";
	std::string replacementFilename = projectName+"/"+projectName+".replace";
	std::string keysFilename = projectName+"/"+projectName+".keys";
    std::string indexFilename = projectName+"/"+projectName+".index";
	
	std::vector<const SeqMine::Sequence*> S;

    Lexicon lexicon = Lexicon(keysFilename.c_str(),replacementFilename.c_str());

 
	bool seqresult = parseSequenceMetaFile (indexFilename.c_str (), S, lexicon);
	if (seqresult == false) {
	...
	if (vm.count ("verbose")) {
			...
			iv = subseq.begin () ; iv != subseq.end () ; ++iv)
		{
			...
			// print out decoded vector
            iv->second.decodeAndPrint(lexicon);

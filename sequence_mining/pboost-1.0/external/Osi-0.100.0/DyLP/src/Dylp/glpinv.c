...
static char svnid[] UNUSED = "$Id: glpinv.c 148 2007-06-09 03:15:30Z lou $" ;
...
{     int *pp_row = inv->luf->pp_row;
      ...
      {  /* the matrix B has been successfully factorized */
         ...
         inv->nnz_h = 0;
	 /* stability is inverse of growth; imperfect, but it'll do for a first
	    cut */
...
{     int m = inv->m;
      ...
      inv->min_vrratio = fabs(vr_piv[i])/(upd_tol*temp) ;
